...
then
  ...
  then
    ...
    then
      ...
      echo "INFO: Starting rclone $RCLONE_CMD $SYNC_SRC $SYNC_DEST $RCLONE_OPTS $SYNC_OPTS_ALL --log-file=${LOG_FILE} --min-age 1m"
      rclone $RCLONE_CMD $SYNC_SRC $SYNC_DEST $RCLONE_OPTS $SYNC_OPTS_ALL --log-file=${LOG_FILE} --min-age 1m
      export RETURN_CODE=$?
    else
      if [ ! -z "$CHECK_URL" ]
      then
